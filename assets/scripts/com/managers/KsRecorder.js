...
let KsRecorder = {
    ...
    setup() {
        debug.log('录屏初始化');
        this.grm = kwaigame.createMediaRecorder();
        debug.log('录屏初始化');
        debug.log(this.grm);
        if (this.grm) {
            debug.log('录屏初始化');
            ...
            this.grm.onStop((res) => {
                ...
                if (WechatAPI.ttRecorder.willShare) {
                    debug.log("！录屏分享");
                    WechatAPI.shareUtil.shareVideo(res.videoID, this.grm);
            ...
            debug.log("createMediaRecorder not suppported");
        }
    },
